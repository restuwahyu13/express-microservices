...
import { JsonWebToken, apiResponse, Middleware, dateFormat, Inject } from '@node/pkg'
  ...
  use(): Handler {
    ...
    return async function (req: Request, res: Response, next: NextFunction): Promise<NextFunction | OutgoingMessage> {
      try {
        ...
        if (!Object.keys(headers).includes('authorization')) throw apiResponse('Roles Service', req.socket.localAddress, status.UNAUTHORIZED, 'Authorization is required')

        const authorization: boolean | undefined = (headers.authorization as string).includes('Bearer')
        if (!authorization) throw apiResponse('Roles Service', req.socket.localAddress, status.UNAUTHORIZED, 'Bearer is required')

        const accessToken: string = (headers.authorization as string).split('Bearer ')[1]
        if (assert.isUndefined(accessToken as any)) throw apiResponse('Roles Service', req.socket.localAddress, status.UNAUTHORIZED, 'Access Token is required')

        const validJwt: string[] = (accessToken as string).split('.')
        if (validJwt?.length !== 3) throw apiResponse('Roles Service', req.socket.localAddress, status.UNAUTHORIZED, 'Access Token format must be jwt')
        ...
        if (!getAccessToken) throw apiResponse('Roles Service', req.socket.localAddress, status.UNAUTHORIZED, 'Access token is not exist')

        if (dateFormat(getAccessToken.expiredAt) < dateFormat(new Date())) throw apiResponse('Roles Service', req.socket.localAddress, status.UNAUTHORIZED, 'Access token expired')
